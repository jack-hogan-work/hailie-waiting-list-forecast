...
def main() -> None:
    ...
    for row in read_csv(FINAL / "national_forecast_2026_2028.csv") + read_csv(FINAL / "regional_forecast_2026_2028.csv"):
        geographies[row["area_code"]]["primary"].append({
            ...
            "lower95": number(row["lower_95"]), "upper95": number(row["upper_95"]),
        })
    for row in read_csv(FINAL / "national_extension_2026_2030.csv") + read_csv(FINAL / "regional_extension_2026_2030.csv"):
        geographies[row["area_code"]]["extension"].append({
            ...
            "lower95": number(row["lower_95"]) if row.get("lower_95") else None,
            "upper95": number(row["upper_95"]) if row.get("upper_95") else None,
        })
...
<style>
:root{--navy:#102f46;--blue:#176d8d;--teal:#16877e;--cyan:#dff5f2;--ink:#14242f;--muted:#526675;--line:#d4dee3;--paper:#fff;--wash:#f1f6f7;--amber:#b56a00}
...
<main id="dashboard">
<section class="intro"><div><div class="eyebrow">Interactive forecast dashboard</div><h1>Where could waiting lists be heading?</h1><p>Explore published local-authority register counts and evidence-led forecasts for England and its nine regions. Use the controls to compare the core three-year outlook with the five-year planning view.</p></div><div class="controls"><label for="geography">Choose an area</label><select id="geography"></select><div class="toggle" role="group" aria-label="Forecast horizon"><button id="core" class="active" type="button">2026–2028 core</button><button id="planning" type="button">2026–2030 planning</button></div></div></section>
<section class="cards" aria-label="Headline figures"><div class="card"><span class="label">Latest published count</span><span class="value" id="latest"></span><span class="note">Households · 2025</span></div><div class="card"><span class="label" id="forecast-label">Forecast for 2028</span><span class="value" id="forecast"></span><span class="note">Central estimate</span></div><div class="card"><span class="label">Change from 2025</span><span class="value" id="change"></span><span class="note" id="direction"></span></div><div class="card"><span class="label">Selected model</span><span class="value" id="model" style="font-size:1.35rem"></span><span class="note">Chosen by rolling-origin MAE</span></div></section>
<p class="answer" id="answer"></p>
<section class="panel"><h2 id="chart-title">Observed and forecast households</h2><p class="panel-intro">Historical counts are shown from 2010. Shaded ranges show empirical 80% and 95% uncertainty where available.</p><div class="chart-wrap"><canvas id="chart" role="img"></canvas></div><div class="legend" aria-hidden="true"><span class="key"><span class="swatch"></span>Observed</span><span class="key"><span class="swatch forecast"></span>Forecast</span><span class="key"><span class="swatch band"></span>80% and 95% ranges</span></div></section>
<section class="grid"><div class="panel"><h2>Forecast detail</h2><div class="table-wrap"><table><caption id="detail-caption"></caption><thead><tr><th>Year</th><th>Central estimate</th><th>80% range</th><th>95% range</th></tr></thead><tbody id="detail-body"></tbody></table></div></div><div class="panel"><h2>Regional picture</h2><p class="panel-intro">Ranked by the 2028 central forecast.</p><div class="table-wrap"><table><caption>Regional social housing waiting-list outlook</caption><thead><tr><th>Region</th><th>2025</th><th>2028</th><th>Change</th></tr></thead><tbody id="ranking"></tbody></table></div></div></section>
<section class="panel"><h2>How to read this forecast</h2><div class="method"><div><strong>Published evidence</strong>MHCLG Live Table 600, covering households on local-authority housing registers from 1987 to 2025.</div><div><strong>Tested models</strong>Seven transparent approaches compared through expanding-window rolling-origin backtesting.</div><div><strong>Uncertainty shown</strong>Empirical prediction ranges are based on errors observed when forecasting unseen historical years.</div></div><p class="source"><strong>Important:</strong> register counts are an administrative measure of social housing waiting-list demand, not a complete measure of housing need. The central forecast is not a target or guarantee. <a href="HAILIE_final_report.html">Read the full report and methodology</a>.</p></section>
</main>
<footer><div><span>HAILIE social housing waiting-list forecast</span><span>Updated __DATE__ · Source: MHCLG Live Table 600</span></div></footer>
<script>
...
function update(){const code=select.value||Object.keys(DATA)[0],item=DATA[code],rows=item[horizon],last=item.history[item.history.length-1],end=rows[rows.length-1],delta=(end.point-last.value)/last.value*100,model=horizon==='primary'?item.models.primary_model_label:item.models.extension_model_label;
document.getElementById('latest').textContent=nf.format(last.value);document.getElementById('forecast').textContent=nf.format(end.point);document.getElementById('forecast-label').textContent=`Forecast for ${end.year}`;document.getElementById('change').textContent=pct(delta);document.getElementById('direction').textContent=Math.abs(delta)<.5?'Broadly stable':delta>0?'Increase indicated':'Decrease indicated';document.getElementById('model').textContent=model;
const tone=Math.abs(delta)<.5?'remain broadly stable':delta>0?'increase':'decrease';document.getElementById('answer').innerHTML=`<strong>${item.name}:</strong> the central estimate indicates waiting-list counts could ${tone} from <strong>${nf.format(last.value)}</strong> in 2025 to <strong>${nf.format(end.point)}</strong> in ${end.year}. Uncertainty widens with the forecast horizon.`;
document.getElementById('detail-caption').textContent=`${item.name} ${rows[0].year}–${end.year} forecast`;document.getElementById('detail-body').innerHTML=rows.map(r=>`<tr><th scope="row">${r.year}</th><td>${nf.format(r.point)}</td><td>${range(r,'lower80')}</td><td>${range(r,'lower95')}</td></tr>`).join('');draw(item,rows)}
function ranking(){const regions=Object.entries(DATA).filter(([c])=>c!=='E92000001').map(([c,i])=>{const y=i.primary.find(r=>r.year===2028),latest=i.history[i.history.length-1].value;return{name:i.name,latest,point:y.point,change:(y.point-latest)/latest*100}}).sort((a,b)=>b.point-a.point);document.getElementById('ranking').innerHTML=regions.map((r,n)=>`<tr><th scope="row"><span class="rank">${n+1}</span> ${r.name}</th><td>${nf.format(r.latest)}</td><td>${nf.format(r.point)}</td><td>${pct(r.change)}</td></tr>`).join('')}
function draw(item,forecast){const canvas=document.getElementById('chart'),box=canvas.parentElement.getBoundingClientRect(),dpr=window.devicePixelRatio||1;canvas.width=box.width*dpr;canvas.height=box.height*dpr;const c=canvas.getContext('2d');c.scale(dpr,dpr);const W=box.width,H=box.height,p={l:68,r:22,t:25,b:48},hist=item.history.filter(x=>x.year>=2010),points=hist.concat(forecast.map(x=>({year:x.year,value:x.point}))),vals=points.map(x=>x.value);forecast.forEach(x=>['lower95','upper95'].forEach(k=>{if(x[k]!=null)vals.push(x[k])}));let min=Math.min(...vals),max=Math.max(...vals),pad=(max-min)*.1||1;min=Math.max(0,min-pad);max+=pad;const x=y=>p.l+(y-2010)/(forecast[forecast.length-1].year-2010)*(W-p.l-p.r),yy=v=>p.t+(max-v)/(max-min)*(H-p.t-p.b);
c.clearRect(0,0,W,H);c.font='12px -apple-system,BlinkMacSystemFont,Segoe UI,sans-serif';c.strokeStyle='#d4dee3';c.fillStyle='#526675';c.textAlign='right';for(let i=0;i<5;i++){const v=min+(max-min)*i/4,Y=yy(v);c.beginPath();c.moveTo(p.l,Y);c.lineTo(W-p.r,Y);c.stroke();c.fillText(nf.format(v),p.l-9,Y+4)}c.textAlign='center';[2010,2015,2020,2025,forecast[forecast.length-1].year].filter((v,i,a)=>a.indexOf(v)===i).forEach(y=>{c.fillText(y,x(y),H-18)});
function band(lo,hi,color){if(forecast.some(r=>r[lo]==null))return;c.beginPath();forecast.forEach((r,i)=>{const X=x(r.year),Y=yy(r[hi]);i?c.lineTo(X,Y):c.moveTo(X,Y)});[...forecast].reverse().forEach(r=>c.lineTo(x(r.year),yy(r[lo])));c.closePath();c.fillStyle=color;c.fill()}band('lower95','upper95','#e3f2f0');band('lower80','upper80','#bee4df');
function line(rows,color,width,dash=[]){c.beginPath();rows.forEach((r,i)=>{const X=x(r.year),Y=yy(r.value??r.point);i?c.lineTo(X,Y):c.moveTo(X,Y)});c.strokeStyle=color;c.lineWidth=width;c.setLineDash(dash);c.stroke();c.setLineDash([])}line(hist,'#176d8d',3);line([{year:2025,point:lastValue(item)},...forecast],'#16877e',3,[7,5]);c.beginPath();c.arc(x(2025),yy(lastValue(item)),4,0,Math.PI*2);c.fillStyle='#176d8d';c.fill();canvas.setAttribute('aria-label',`${item.name} chart showing observed register counts from 2010 to 2025 and forecasts to ${forecast[forecast.length-1].year}`)}
function lastValue(item){return item.history[item.history.length-1].value}select.addEventListener('change',update);document.getElementById('core').onclick=()=>{horizon='primary';document.getElementById('core').classList.add('active');document.getElementById('planning').classList.remove('active');update()};document.getElementById('planning').onclick=()=>{horizon='extension';document.getElementById('planning').classList.add('active');document.getElementById('core').classList.remove('active');update()};window.addEventListener('resize',update);ranking();update();
</script>
</body></html>'''
