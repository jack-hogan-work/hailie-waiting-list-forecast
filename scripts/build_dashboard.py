...
def main() -> None:
    ...
    selections = {
        row["area_code"]: {**row, "extension_y5_mae": row["y5_mae_households"]}
        for row in read_csv(FINAL / "regional_model_selection.csv")
    }
    ...
    national_primary = next(row for row in national_selections if row["forecast_role"] == "primary_2026_2028")
    national_extension = next(row for row in national_selections if row["forecast_role"] == "extension_2026_2030")
    selections[ENGLAND] = {
        "primary_model": national_primary["selected_model"],
        "primary_model_label": national_primary["selected_model_label"],
        "extension_model": national_extension["selected_model"],
        "extension_model_label": national_extension["selected_model_label"],
        "extension_y5_mae": national_extension["selection_score_households"],
        "extension_selection_note": national_extension["selection_note"],
    }
...
.answer{font-size:1.08rem;border-left:5px solid var(--teal);padding:.8rem 1rem;background:var(--cyan);margin:1.2rem 0}.grid{display:grid;grid-template-columns:1fr 1fr;gap:1.2rem}.grid>.panel{min-width:0}.table-wrap{overflow:auto}table{width:100%;border-collapse:collapse;font-size:.92rem}caption{text-align:left;font-weight:750;color:var(--navy);margin-bottom:.6rem}th,td{padding:.7rem;border-bottom:1px solid var(--line);text-align:right;white-space:nowrap}th:first-child,td:first-child{text-align:left}thead th{background:var(--navy);color:#fff}.rank{font-weight:750;color:var(--navy)}
...
<main id="dashboard">
<section class="intro"><div><div class="eyebrow">Interactive forecast dashboard</div><h1>Social housing waiting-list forecast for England</h1><p>Explore published local-authority register counts and evidence-led forecasts for England and its nine regions. Use the controls to compare the core three-year outlook with the five-year forecast. The five-year view is a cautious planning extension: a flat line means the lowest-error model carried 2025 forward, not that waiting lists will remain unchanged.</p></div><div class="controls"><label for="geography">Choose an area</label><select id="geography"></select><div class="toggle" role="group" aria-label="Forecast horizon"><button id="core" class="active" aria-pressed="true" type="button">3-year forecast</button><button id="planning" aria-pressed="false" type="button">5-year forecast</button></div></div></section>
<section class="cards" aria-label="Headline figures" aria-live="polite"><div class="card"><span class="label">Latest published count</span><span class="value" id="latest"></span><span class="note">Households · 2025</span></div><div class="card"><span class="label" id="forecast-label">Forecast for 2028</span><span class="value" id="forecast"></span><span class="note" id="forecast-note">Central estimate</span></div><div class="card"><span class="label">Change from 2025</span><span class="value" id="change"></span><span class="note" id="direction"></span></div><div class="card"><span class="label">Selected model</span><span class="value" id="model" style="font-size:1.35rem"></span><span class="note" id="model-note">Chosen by rolling-origin MAE</span></div></section>
<p class="answer" id="answer" aria-live="polite"></p>
<section class="panel" id="national-sensitivity" hidden><h2>National history-window sensitivity</h2><p class="panel-intro">Naive has lower mean Y1–Y3 MAE than damped Holt in both later-history windows. This weakens the directional conclusion without changing the full-history central estimate.</p><div class="table-wrap"><table><caption>Damped-Holt performance when validation starts later; MAE in households</caption><thead><tr><th scope="col">History window</th><th scope="col">Damped-Holt mean MAE</th><th scope="col">Rank</th><th scope="col">Naive mean MAE</th><th scope="col">Window winner</th></tr></thead><tbody id="sensitivity-body"></tbody></table></div></section>
<section class="panel"><h2 id="chart-title">Observed and forecast households</h2><p class="panel-intro">Historical counts are shown from 2010. Shading shows the empirical 80% range where available. Wider 95% diagnostic ranges are retained only in the <a href="HAILIE_final_report.html">technical report</a> because their tails depend on very few extreme observations.</p><div class="chart-wrap"><canvas id="chart" role="img" aria-labelledby="chart-title" aria-describedby="chart-key"></canvas></div><div class="legend" id="chart-key"><span class="key"><span class="swatch"></span>Observed</span><span class="key"><span class="swatch forecast"></span>Forecast</span><span class="key"><span class="swatch band"></span>80% range</span></div></section>
<section class="grid"><div class="panel"><h2>Forecast detail</h2><div class="table-wrap"><table><caption id="detail-caption"></caption><thead><tr><th scope="col">Year</th><th scope="col">Central estimate</th><th scope="col">80% range</th></tr></thead><tbody id="detail-body"></tbody></table></div></div><div class="panel"><h2>England and regional picture (alphabetical)</h2><p class="panel-intro">Six of nine regions use a naive model that carries 2025 forward. These repeated central estimates are model properties, not evidence of stability. Each estimate is paired with its 80% range. Source-noted breaks include Telford &amp; Wrekin leaving the register series from 31 March 2021 and Epping Forest changing transfer-applicant treatment from 2022–23.</p><div class="table-wrap"><table><caption>England and regional models and 2028 social housing waiting-list outlook, shown alphabetically</caption><thead><tr><th scope="col">Area</th><th scope="col">Model</th><th scope="col">2025</th><th scope="col">2028 central</th><th scope="col">2028 80% range</th></tr></thead><tbody id="ranking"></tbody></table></div></div></section>
...
function pct(v){return `${v>=0?'+':''}${v.toFixed(1)}%`}function range(row,key){return row[key]==null?'—':`${nf.format(row[key])}–${nf.format(row[key.replace('lower','upper')])}`}function niceStep(span){const rough=span/4,power=10**Math.floor(Math.log10(rough)),scaled=rough/power,multiplier=scaled<=1?1:scaled<=2?2:scaled<=2.5?2.5:scaled<=5?5:10;return multiplier*power}
function update(){const code=select.value||Object.keys(DATA)[0],item=DATA[code],rows=item[horizon],last=item.history[item.history.length-1],end=rows[rows.length-1],delta=(end.point-last.value)/last.value*100,model=horizon==='primary'?item.models.primary_model_label:item.models.extension_model_label,nationalCore=code==='E92000001'&&horizon==='primary',regionalCore=code!=='E92000001'&&horizon==='primary',naiveCarry=regionalCore&&item.models.primary_model==='naive',fiveYear=horizon==='extension';
document.getElementById('latest').textContent=nf.format(last.value);document.getElementById('forecast').textContent=nf.format(end.point);document.getElementById('forecast-label').textContent=`Forecast for ${end.year}`;document.getElementById('forecast-note').textContent=nationalCore?'Full-history central estimate':regionalCore?'Selected-model central estimate':fiveYear?'Cautious planning estimate':'Planning point estimate';document.getElementById('change').textContent=pct(delta);document.getElementById('direction').textContent=nationalCore?'Full-history estimate':naiveCarry?'Naive carry-forward':regionalCore?'Modelled difference':'No directional call';document.getElementById('model').textContent=model;document.getElementById('model-note').textContent=fiveYear?'Chosen by five-year backtest MAE':'Chosen by rolling-origin MAE';document.getElementById('chart-title').textContent=`${item.name}: observed and forecast households`;
const sensitivityPanel=document.getElementById('national-sensitivity');if(nationalCore){const later=item.sensitivity[item.sensitivity.length-1];document.getElementById('answer').innerHTML=`<strong>England:</strong> the evidence does not identify a robust national increase or decrease over the next three years. The full-history damped-Holt central estimate is <strong>${nf.format(end.point)} in ${end.year} (${pct(delta)})</strong>, but naive performs better in both later-history sensitivity windows, especially ${later.window}.`;document.getElementById('sensitivity-body').innerHTML=item.sensitivity.map(r=>`<tr><th scope="row">${r.window}</th><td>${nf.format(r.dampedMae)}</td><td>${r.dampedRank} of 7</td><td>${nf.format(r.naiveMae)}</td><td>${r.winner}</td></tr>`).join('');sensitivityPanel.hidden=false}else if(regionalCore){const modelContext=naiveCarry?`The central estimate repeats the 2025 observation because backtesting selected the naive carry-forward model. This is a property of the model, not evidence that the register count will remain unchanged.`:`The selected ${model} model gives a central change of ${pct(delta)} from 2025.`;document.getElementById('answer').innerHTML=`<strong>${item.name}:</strong> ${modelContext} The ${end.year} central estimate is <strong>${nf.format(end.point)}</strong>, with an 80% range of <strong>${range(end,'lower80')}</strong>. Regional backtests do not support a strong directional call.`;sensitivityPanel.hidden=true}else if(fiveYear){const carriesForward=item.models.extension_model==='naive',englandTie=code==='E92000001'&&carriesForward;const modelContext=englandTie?`The naive model was equal-lowest on five-year backtest MAE (${nf.format(Number(item.models.extension_y5_mae))} households); the documented parsimony tie-break selected it over an equivalent ARIMA result. It carries the 2025 count forward, making the central line flat. This is not a prediction that waiting-list demand will remain unchanged.`:carriesForward?`The naive model had the lowest five-year backtest MAE (${nf.format(Number(item.models.extension_y5_mae))} households) and carries the 2025 count forward. This makes the central line flat; it is not a prediction that waiting-list demand will remain unchanged.`:`The selected ${model} model had the lowest five-year backtest MAE (${nf.format(Number(item.models.extension_y5_mae))} households). Its central path is a cautious longer-horizon planning estimate, not a directional call.`;const uncertainty=code==='E92000001'?`It includes an empirical 80% range, which widens with the forecast horizon.`:`No uncertainty was computed for these regional five-year points, so they should not be used as standalone planning forecasts.`;document.getElementById('answer').innerHTML=`<strong>${item.name}:</strong> ${modelContext} ${uncertainty}`;sensitivityPanel.hidden=true}
...
function ranking(){const areas=Object.entries(DATA).map(([c,i])=>{const y=i.primary.find(r=>r.year===2028),latest=i.history[i.history.length-1].value;return{name:i.name,model:i.models.primary_model_label,latest,point:y.point,lower:y.lower80,upper:y.upper80}}).sort((a,b)=>a.name.localeCompare(b.name));document.getElementById('ranking').innerHTML=areas.map(r=>`<tr><th scope="row">${r.name}</th><td>${r.model}</td><td>${nf.format(r.latest)}</td><td>${nf.format(r.point)}</td><td>${nf.format(r.lower)}–${nf.format(r.upper)}</td></tr>`).join('')}
function draw(item,forecast){const canvas=document.getElementById('chart'),box=canvas.parentElement.getBoundingClientRect(),dpr=window.devicePixelRatio||1;canvas.width=box.width*dpr;canvas.height=box.height*dpr;const c=canvas.getContext('2d');c.scale(dpr,dpr);const W=box.width,H=box.height,p={l:112,r:22,t:20,b:74},hist=item.history.filter(x=>x.year>=2010),points=hist.concat(forecast.map(x=>({year:x.year,value:x.point}))),vals=points.map(x=>x.value);forecast.forEach(x=>['lower80','upper80'].forEach(k=>{if(x[k]!=null)vals.push(x[k])}));let min=Math.min(...vals),max=Math.max(...vals),pad=(max-min)*.1||1;min=Math.max(0,min-pad);max+=pad;const yStep=niceStep(max-min);min=Math.max(0,Math.floor(min/yStep)*yStep);max=Math.ceil(max/yStep)*yStep;const endYear=forecast[forecast.length-1].year,plotBottom=H-p.b,x=y=>p.l+(y-2010)/(endYear-2010)*(W-p.l-p.r),yy=v=>p.t+(max-v)/(max-min)*(plotBottom-p.t);c.font='12px -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif';c.fillStyle='#4f5f6c';c.strokeStyle='#d5dfe4';c.lineWidth=1;for(let value=min;value<=max+yStep/2;value+=yStep){const Y=yy(value);c.beginPath();c.moveTo(p.l,Y);c.lineTo(W-p.r,Y);c.stroke();c.textAlign='right';c.textBaseline='middle';c.fillText(nf.format(Math.round(value)),p.l-8,Y)}const xTicks=[...new Set([2010,2015,2020,2025,endYear])].filter(year=>year>=2010&&year<=endYear).sort((a,b)=>a-b);xTicks.forEach(year=>{const X=x(year);c.beginPath();c.moveTo(X,p.t);c.lineTo(X,plotBottom);c.stroke();c.textAlign='center';c.textBaseline='top';c.fillText(String(year),X,plotBottom+10)});c.strokeStyle='#718995';c.beginPath();c.moveTo(p.l,p.t);c.lineTo(p.l,plotBottom);c.lineTo(W-p.r,plotBottom);c.stroke();c.fillStyle='#304b5d';c.font='600 12px -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif';c.textAlign='center';c.textBaseline='alphabetic';c.fillText('Year',(p.l+W-p.r)/2,H-16);c.save();c.translate(18,H/2);c.rotate(-Math.PI/2);c.fillText('Households on local-authority housing registers',0,0);c.restore();
