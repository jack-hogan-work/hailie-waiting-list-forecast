...
- **No forward (2026-2030) regional forecast produced.** Unlike the national work, this extension does not include an illustrative future-year forecast table — it stops at backtesting. Adding one would be straightforward (the same `forward_forecast()`-style logic from `forecast_national.py` could be reused per region) but was judged out of scope for this pass.
